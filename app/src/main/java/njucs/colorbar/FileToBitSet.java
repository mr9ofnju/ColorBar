...
import njucs.colorbar.ReedSolomon.GenericGF;
import njucs.colorbar.ReedSolomon.ReedSolomonEncoder;
...
public class FileToBitSet {
    ...
    private List<byte[]> readFile(byte[] byteData) {
        ...
        final int realByteLength = bitsPerBlock* contentWidth * contentHeight / 8 - ecNum * ecLength / 8 - 8;
    ...
    private BitSet[] RSEncode(String filePath) {
        ...
        for (byte[] b : byteBuffer) {
            int[] ordered = new int[(int)Math.ceil((double)bitsPerBlock* contentWidth * contentHeight / ecLength)];
            ...
            BitSet current=toBitSet(ordered, ecLength,bitsPerBlock*contentWidth*contentHeight-ecLength*ecNum);
